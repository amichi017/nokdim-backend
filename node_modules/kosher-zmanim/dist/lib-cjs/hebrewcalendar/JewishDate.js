...
class JewishDate {
    constructor(jewishYearOrDateTimeOrDateOrMolad, jewishMonth, jewishDayOfMonth) {
        if (!jewishYearOrDateTimeOrDateOrMolad) {
            ...
            this.setDate(luxon_1.DateTime.fromJSDate(jewishYearOrDateTimeOrDateOrMolad));
        }
        else if (luxon_1.DateTime.isDateTime(jewishYearOrDateTimeOrDateOrMolad)) {
    ...
    static validateJewishDate(year, month, dayOfMonth, hours, minutes, chalakim) {
        if (month < JewishDate.NISSAN || month > JewishDate.getLastMonthOfJewishYear(year)) {
            throw new errors_1.IllegalArgumentException(`The Jewish month has to be between 1 and 12 (or 13 on a leap year). ${month} is invalid for the year ${year}.`);
        }
        if (dayOfMonth < 1 || dayOfMonth > 30) {
            throw new errors_1.IllegalArgumentException(`The Jewish day of month can't be < 1 or > 30. ${dayOfMonth} is invalid.`);
        }
        ...
        if ((year < 3761) || (year === 3761 && (month >= JewishDate.TISHREI && month < JewishDate.TEVES))
            ...
            throw new errors_1.IllegalArgumentException(`A Jewish date earlier than 18 Teves, 3761 (1/1/1 Gregorian) can't be set. ${year}, ${month}, ${dayOfMonth} is invalid.`);
        }
        if (hours < 0 || hours > 23) {
            throw new errors_1.IllegalArgumentException(`Hours < 0 or > 23 can't be set. ${hours} is invalid.`);
        }
        if (minutes < 0 || minutes > 59) {
            throw new errors_1.IllegalArgumentException(`Minutes < 0 or > 59 can't be set. ${minutes} is invalid.`);
        }
        if (chalakim < 0 || chalakim > 17) {
            throw new errors_1.IllegalArgumentException(`Chalakim/parts < 0 or > 17 can't be set. ${chalakim} is invalid. For larger numbers such as 793 (TaShTzaG) break the chalakim into minutes (18 chalakim per minutes, so it would be 44 minutes and 1 chelek in the case of 793 (TaShTzaG)`);
        }
    }
    /**
    ...
    static validateGregorianMonth(month) {
        if (month > 11 || month < 0) {
            throw new errors_1.IllegalArgumentException(`The Gregorian month has to be between 0 - 11. ${month} is invalid.`);
        }
    }
    /**
    ...
    static validateGregorianDayOfMonth(dayOfMonth) {
        if (dayOfMonth <= 0) {
            throw new errors_1.IllegalArgumentException(`The day of month can't be less than 1. ${dayOfMonth} is invalid.`);
        }
    }
    /**
    ...
    static validateGregorianYear(year) {
        if (year < 1) {
            throw new errors_1.IllegalArgumentException(`Years < 1 can't be calculated. ${year} is invalid.`);
        }
    }
    /**
    ...
    getMolad() {
        ...
        if (moladDate.getMoladHours() >= 6) {
            moladDate.forward(Utils_1.Calendar.DATE, 1);
        }
    ...
    */
    ...
    setDate(date) {
        if (date.year < 1) {
            throw new errors_1.IllegalArgumentException(`Dates with a BC era are not supported. The year ${date.year} is invalid.`);
        }
    ...
    */
     ...
     */
    resetDate() {
        this.setDate(luxon_1.DateTime.local());
    }
    /**
    ...
    forward(field, amount) {
        if (field !== Utils_1.Calendar.DATE && field !== Utils_1.Calendar.MONTH && field !== Utils_1.Calendar.YEAR) {
            throw new errors_1.IllegalArgumentException('Unsupported field was passed to Forward. Only Calendar.DATE, Calendar.MONTH or Calendar.YEAR are supported.');
        }
        if (amount < 1) {
            throw new errors_1.IllegalArgumentException('JewishDate.forward() does not support amounts less than 1. See JewishDate.back()');
        }
        if (field === Utils_1.Calendar.DATE) {
        ...
        else if (field === Utils_1.Calendar.MONTH) {
            this.forwardJewishMonth(amount);
        }
        else if (field === Utils_1.Calendar.YEAR) {
    ...
    forwardJewishMonth(amount) {
        if (amount < 1) {
            throw new errors_1.IllegalArgumentException('the amount of months to forward has to be greater than zero.');
        }
        for (let i = 0; i < amount; i++) {
    ...
    compareTo(jewishDate) {
        return Utils_1.IntegerUtils.compare(this.gregorianAbsDate, jewishDate.getAbsDate());
    }
    /**
