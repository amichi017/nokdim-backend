...
class AstronomicalCalendar {
    ...
    */
     ...
     */
    constructor(geoLocation = new GeoLocation_1.GeoLocation()) {
        this.setDate(luxon_1.DateTime.fromObject({ zone: geoLocation.getTimeZone() }));
        this.setGeoLocation(geoLocation); // duplicate call
        this.setAstronomicalCalculator(new NOAACalculator_1.NOAACalculator());
    }
    /**
    ...
    static getTimeOffset(time, offset) {
        if (time === null || offset === Utils_1.Long_MIN_VALUE || Number.isNaN(offset)) {
            return null;
        }
    ...
    */
    ...
    getDateFromTime(time, isSunrise) {
        ...
        let cal = luxon_1.DateTime.utc(adjustedDate.year, adjustedDate.month, adjustedDate.day);
    ...
    getSunriseSolarDipFromOffset(minutes) {
        ...
        let degrees = new big_js_1.Big(0);
        const incrementor = new big_js_1.Big('0.0001');
    ...
    getSunsetSolarDipFromOffset(minutes) {
        ...
        let degrees = new big_js_1.Big(0);
        const incrementor = new big_js_1.Big('0.001');
        // If `minutes` is not `NaN` and `offsetByDegrees` is not null, `offsetByTime` should not be null
    ...
    toString() {
        throw new errors_1.UnsupportedError('This method is unsupported, due to the fact that it depends on a circular dependency.');
    }
    /**
    ...
    setDate(date) {
        if (luxon_1.DateTime.isDateTime(date)) {
            ...
            this.date = luxon_1.DateTime.fromJSDate(date);
        }
        else if (typeof date === 'string') {
            this.date = luxon_1.DateTime.fromISO(date);
        }
        else if (typeof date === 'number') {
            this.date = luxon_1.DateTime.fromMillis(date);
        }
    }
    /**
