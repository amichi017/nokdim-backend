...
exports.GeoLocationUtils = void 0;
...
class GeoLocationUtils {
    ...
    static vincentyFormula(location, destination, formula) {
        ...
        const L = Utils_1.MathUtils.degreesToRadians(destination.getLongitude() - location.getLongitude()); // difference in longitude of two points;
        const U1 = Math.atan((1 - f) * Math.tan(Utils_1.MathUtils.degreesToRadians(location.getLatitude()))); // reduced latitude (latitude on the auxiliary sphere)
        const U2 = Math.atan((1 - f) * Math.tan(Utils_1.MathUtils.degreesToRadians(destination.getLatitude()))); // reduced latitude (latitude on the auxiliary sphere)
        ...
        // initial bearing
        const fwdAz = Utils_1.MathUtils.radiansToDegrees(Math.atan2(cosU2 * sinLambda, cosU1
            * sinU2 - sinU1 * cosU2 * cosLambda));
        // final bearing
        const revAz = Utils_1.MathUtils.radiansToDegrees(Math.atan2(cosU1 * sinLambda, -sinU1
    ...
    static getRhumbLineBearing(location, destination) {
        let dLon = Utils_1.MathUtils.degreesToRadians(destination.getLongitude() - location.getLongitude());
        const dPhi = Math.log(Math.tan(Utils_1.MathUtils.degreesToRadians(destination.getLatitude())
            / 2 + Math.PI / 4)
            / Math.tan(Utils_1.MathUtils.degreesToRadians(location.getLatitude()) / 2 + Math.PI / 4));
    ...
    static getRhumbLineDistance(location, destination) {
        ...
        const dLat = Utils_1.MathUtils.degreesToRadians(location.getLatitude())
            - Utils_1.MathUtils.degreesToRadians(destination.getLatitude());
        let dLon = Math.abs(Utils_1.MathUtils.degreesToRadians(location.getLongitude())
            - Utils_1.MathUtils.degreesToRadians(destination.getLongitude()));
        const dPhi = Math.log(Math.tan(Utils_1.MathUtils.degreesToRadians(location.getLatitude()) / 2 + Math.PI / 4)
            / Math.tan(Utils_1.MathUtils.degreesToRadians(destination.getLatitude()) / 2 + Math.PI / 4));
        let q = dLat / dPhi;
        if (!Number.isFinite(q)) {
            q = Math.cos(Utils_1.MathUtils.degreesToRadians(destination.getLatitude()));
        }
